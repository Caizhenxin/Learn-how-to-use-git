...
app = FastAPI(title="AutoTranslation")
deepl_auth_key = "5225af3e-9652-80ce-c74e-307ead3e9880:fx"  # Replace with your key
...
def translate(req: TranslationRequest):
    ...
    collection = parseString(req.annotations).documentElement
    # 生成翻译参考词汇表
    entries = {}
...
def truncate_string(s, pos):
    pos = len(s) + pos if pos < 0 else pos  # 处理负数位置
    if len(s) <= pos:
        return s
    ...
    # 返回最近的换行符前的部分
    return s[:before_newline] if before_newline != -1 else s[:pos + after_newline] if after_newline != -1 else s
...
def LLMchat(req: LLMRequest):
    ...
    if chat_bot is None or chat_bot.is_driver_quitted():
        # Create a chatbot
        chat_bot = ChatGPTAutomation(chrome_path=cwd+'/chrome-win64/chrome.exe')
    elif req.new_chat:
        chat_bot.new_chat()
    ...
    while True:
        ...
        response_cur = chat_bot.return_last_response_md()
