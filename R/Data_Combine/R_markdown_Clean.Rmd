...
rm(sleep_duration,sleep_duration_score,calculate_sleep_duration_score)
```
...
#计算  床上时间=起床时间（条目3）—上床时间（条目1）
    ...
    # 计算时间差（单位：小时）
    psqi_data$IV_score <- as.numeric(difftime(psqi_data$wakeup_time, psqi_data$bedtime, units = "hours"))
...
# 从已有变量中筛选相关变量
psqi_scores <- psqi_data[, c("I_score", "II_score", "III_score", "IV_score", "V_score", "VI_score", "VII_score", "id")]

# 计算PSQI总分
psqi_scores$PSQI_total <- rowSums(psqi_scores[, -ncol(psqi_scores)], na.rm = TRUE)
